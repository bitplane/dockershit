...
@pytest.fixture
def tmp_dockerfile_path(tmp_path) -> str:
    """Create a temporary Dockerfile path."""
    return tmp_path / "Dockerfile"


@pytest.fixture
def empty_dockerfile(tmp_dockerfile_path: str) -> Dockerfile:
    """Create a fresh, empty Dockerfile."""
    return Dockerfile(tmp_dockerfile_path)


@pytest.fixture
def simple_dockerfile(tmp_dockerfile_path: str) -> Dockerfile:
...
@pytest.fixture
def populated_dockerfile(tmp_dockerfile_path: str) -> Dockerfile:
...
@pytest.fixture
def multiline_dockerfile(tmp_dockerfile_path: str) -> Dockerfile:
...
@pytest.fixture
def indented_dockerfile(tmp_dockerfile_path: str) -> Dockerfile:
...
def test_cd(empty_dockerfile):
    """Test setting the working directory."""
    empty_dockerfile.cd("/usr/src/app")
...
def test_remove_last_command(simple_dockerfile: Dockerfile):
    ...
    assert "# (removed) RUN echo world" in simple_dockerfile.path.read_text()
...
def test_remove_last_command_with_trailing_meaningless(simple_dockerfile):
    ...
    assert "# (removed) RUN echo hello" in simple_dockerfile.lines
    assert "# comment 1" in simple_dockerfile.lines
    assert "" not in simple_dockerfile.lines
    assert "# comment 2" in simple_dockerfile.lines
